...
describe('CFF Runtime Fidelity: Sandbox & Limits', () => {
    ...
    let runner: any = null; // Store current runner here
    ...
    test('🔄 CFF Event Mapping: Bidirectional validation', () => {
        ...
        const event = runner.toCFFEvent(req, resData, 'viewer-response');
        ...
        expect(mappedRes.headers['x-resp'].value).toBe('hello');
    });
