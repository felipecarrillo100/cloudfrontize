...
const { HeaderManager } = require('../src/core/HeaderManager');
...
describe('Header Compliance: Case Preservation & Multi-Value Arrays', () => {
    ...
    describe('Case Preservation', () => {
        test('📋 Should preserve original header casing in key property', () => {
            ...
            const parsed = manager.parseIncomingHeaders(req);
    ...
    describe('Multi-Value Headers', () => {
        ...
        test('🔁 normalizeHeaders: correctly maps existing AWS header array to output', () => {
            const manager = new HeaderManager();
            ...
            const normalized = manager.normalizeHeaders(lambdaReturnHeaders);
        ...
        test('🔁 normalizeHeaders: handles single-value string entries', () => {
            const manager = new HeaderManager();
            ...
            const normalized = manager.normalizeHeaders(input);

            expect(normalized['content-type']).toHaveLength(1);
            expect(normalized['content-type'][0].value).toBe('application/json');
        });

        test('🔁 normalizeHeaders: handles native string arrays (multi-value strings)', () => {
            const manager = new HeaderManager();
