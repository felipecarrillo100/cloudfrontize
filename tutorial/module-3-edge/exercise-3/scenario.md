...
### Security Filtering: The Edge WAF
This pattern is perfect for building a lightweight Web Application Firewall (WAF) directly in code. As a CloudFront Architect, you can use body inspection to block known malicious payloads (SQL Injection, XSS, specific bot signatures) at the edge PoP, ensuring they never reach your backend application.
...
2.  Trigger a malicious request using `curl`:
    ```bash
    curl -X POST -d "SQL-INJECTION" http://localhost:3000/
    ```
3.  In the traffic list, click the request row to expand the **Execution Journey**.
4.  In the journey tree, click the **[L@E: viewer-request]** station. Notice the **Short-Circuit** behavior: the request highway ends here because you returned a `403` response.
5.  In the **State Inspector** panel, select the **Body** tab. Click **DECODE** to see the "SQL-INJECTION" payload that was neutralized.

### 3. Verification with `curl`
*   **Safe Request**: `curl -X POST -d "user=john" http://localhost:3000/` -> **200 OK**
*   **Malicious Request**: `curl -X POST -d "SQL-INJECTION" http://localhost:3000/` -> **403 Forbidden**
