    ...
    <div key={r.id} style={{ background: '#161b22', borderRadius: 8, border: `1px solid ${isExpanded ? '#f97316' : '#30363d'}` }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ width: 50, fontSize: '0.7rem', fontWeight: 700, color: statusColor(r.status, r.isError) }}>
            {r.status || '...'}
          </span>
...
export default function TrafficCenter() {
  ...
  const filteredRequests = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter(r => r.url?.toLowerCase().includes(s) || String(r.status).includes(s));
    }
